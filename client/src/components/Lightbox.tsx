...
type LightboxProps = { photo: Photo; folderPath?: string; onOpenFolder?: () => void; onClose: () => void; onPrevious: () => void; onNext: () => void };

export function Lightbox({ photo, folderPath, onOpenFolder, onClose, onPrevious, onNext }: LightboxProps) {
  ...
  return (
    <div ref={lightboxRef} className={`lightbox${isClosing ? " is-closing" : ""}`} role="dialog" aria-modal="true" aria-label={`Xem thiết kế ${photoTitle}`}>
      <div className="lightbox__bar">
        <div><p className="eyebrow">Thiết kế</p><h2>{photoTitle}</h2></div>
