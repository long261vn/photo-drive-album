...
export function LiturgicalFilters({ filters, seasons, years, weeks, onChange }: LiturgicalFiltersProps) {
  const hasActiveFilter = Boolean(filters.season || filters.liturgicalYear || filters.week || filters.saintsOnly || filters.marianOnly || filters.childrenOnly);
  return <div className="liturgical-filters" aria-label="Lọc theo lịch phụng vụ">
    ...
    <button className={`liturgical-filters__chip${filters.childrenOnly ? " is-active" : ""}`} type="button" onClick={() => onChange({ ...filters, childrenOnly: !filters.childrenOnly })} aria-pressed={filters.childrenOnly}>Thiếu Nhi</button>
    {hasActiveFilter && <button className="liturgical-filters__reset" type="button" onClick={() => onChange({ season: "", liturgicalYear: "", week: "", saintsOnly: false, marianOnly: false, childrenOnly: false })}><RotateCcw size={13} strokeWidth={1.8} /> Bỏ lọc</button>}
  </div>;
}
